...
## Colors, these come from the khroma pacakge ("muted")
### For <=9 stages:
...
```{r}
#| content: valuebox
#| title: "Total bases sequenced"
#| icon: magic
#| color: primary
nanoq_reports %>% filter(stat == "Bases") %$% sum(val) %>% format(scientific=-1,trim=T, digits = 3, drop0trailing=T)
```
...
#| icon: collection
#| color: secondary	
nanoq_reports %>% filter(stat == "N Reads") %$% sum(val,na.rm = T) %>% paste0()
```
...
#| color: success
nanoq_reports %>% filter(stat == "Longest") %$% max(val) %>% paste(" bases")
```
...
p <- nanoq_reports %>% 
  ...
  facet_wrap(~stat, scales = "free_y", nrow=2, ncol=3) +
  fill_scale_plots +
...
ggplotly(p) %>% plotly::layout(font = plotly_fnt)
```
...
quast_stats <- list.files(paste0(data_base, "quast"),
  ...
  mutate(
    ...
    stage = case_when(
      ...
      str_detect(assembly, ".assembly") ~ "Assembly",
      TRUE ~ "Unknown"
    )
  )
```
...
#| color: primary
    quast_stats %>%
          ...
          unique() %$%
          glue::glue("{format(value,
          scientific = -1,
          trim = T,
          digits = 3,
          drop0trailing = T
          ) %>%
          paste('bp')} in: {sample %>% paste(collapse='')}")
```
...
```{r quast plot}
#| eval: true
#| title: "Quast summary statistics"

p <- quast_stats %>% 
  filter(stat %in% c("Total length", "Largest contig", "GC (%)", "N50")) %>% 
  mutate(stat=fct_relevel(stat,c("Total length", "Largest contig", "GC (%)", "N50"))) %>% 
  ggplot(aes(x = sample, y = value)) +
  geom_line() +
  geom_point(size = 5, pch=21, aes(fill=stage), position=position_dodge(width=0.4)) +
  facet_wrap(~stat, scales = "free_y", nrow=2, ncol=3) +
  fill_scale_plots +
  scale_y_continuous(labels = function(x) format(x,scientific=-1,trim=T, digits = 3, drop0trailing=T), n.breaks = 4) +
  theme(axis.title.x = element_blank(),
...
for (i in 1:length(unique(busco_reports$sample))) {
  ...
  cat(paste('### ', unique(busco_reports$sample)[i]),
      ...
      paste('#### { .flow }'),
      paste0('\n\n'),
      knitr::knit_child(glue::glue('busco_files/orthologs/_{ unique(nanoq_reports$sample)[i] }_orthologs.Rmd'), 
                        envir = globalenv(), 
                        quiet = TRUE),
      paste0('\n\n\n'),
      sep = "")
...
```{r}
#| content: valuebox
#| title: "Largest estimated haploid genome"
#| icon: flower3
#| color: success
genomescope_out %>% filter(property == "Genome Haploid Length")  %>% filter(max == max(max)) %$% sample
```

```{r}
#| content: valuebox
#| title: "Average heterozygosity"
#| icon: person-arms-up
#| color: info
genomescope_out %>% filter(property == "Heterozygosity") %$% mean(c(min, max)) %>%  round(3) %>% paste("%")
...
#| icon: emoji-frown
#| color: danger
genomescope_out %>% filter(property == "Read Error Rate") %$% mean(c(min, max)) %>% round(3) %>% paste("%")
```

## Genomescope plots {.flow}

```{r}
#| output: asis
img_files <- list.files("examples/genomescope", full.names = T, pattern = "plot.png")
cat(":::{.panel-tabset}\n",
...
merqury_stats <- list.files(paste0(data_base, "merqury"), full.names = T, pattern = "stats") %>% 
  ...
  bind_rows() %>% 
  mutate(sample = str_extract(sample_stage, ".+?(?=.assembly|_pilon|_medaka)"),
         stage = case_when(
                  ...
                  str_detect(sample_stage, ".assembly") ~ "Assembly",
                  TRUE ~ "Unknown"))
merqury_asm_hists <- list.files(paste0(data_base, "/merqury"), full.names = T, pattern = "asm.hist")  %>% 
  lapply(\(x) read_tsv(x, col_names = T, show_col_types = FALSE) %>% 
           mutate(
             sample = str_extract(x %>% basename(), ".+?(?=_assemble|_polish|_run|.spectra)"),
             stage = case_when(
               ...
               str_detect(x, "_assemble") ~ "Assembly",
               TRUE ~ "Unknown"),
...
for (i in 1:length(unique(merqury_stats$sample))) {
  ...
  lowest_stage  <-  merqury_stats %>% 
    ...
    stage %>% 
    unique()
  img_files <- list.files("examples/merqury", full.names = T, pattern = "spectra-cn.fl.png")  %>% 
