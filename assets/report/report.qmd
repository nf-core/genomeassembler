...
# groups
groups <- yaml::read_yaml("examples/yml/example.yml") %>%
  map_dfr(\(row)
          data.frame(sample = pluck(row, 1, "id"), group = pluck(row, 1, "group"))) %>%
...
paste0('```{r}\n
        ...
        ggplotly(p)\n```') %>%
    write_lines(glue::glue("nanoq_files/_{ unique(nanoq_reports$group)[i] }_nanoq.Rmd"))
}
```
...
for (i in 1:length(unique(nanoq_reports$group))) {
    cat(paste0('## ', unique(nanoq_reports$group)[i], '\n\n'),
        paste0('### { width = 30% }', '\n\n'),
        ...
        paste0(':::', '\n\n'),
        paste0('### ', '\n\n'),
        knitr::knit_child(glue::glue('nanoq_files/_{ unique(nanoq_reports$group)[i] }_nanoq.Rmd'),
                        envir = globalenv(),
                        quiet = TRUE),
        paste0('\n\n'),
        sep = ""
