...
merqury_stats <- list.files(paste0(data_base, "merqury"), full.names = T, pattern = "stats") %>% 
  ...
  bind_rows() %>% 
  mutate(sample = str_extract(sample_stage,
                              ".+?(?=_assemble|\\.assembly|\\.asm|_medaka|_pilon|_links|\\.tigmint)"),
         stage = case_when(
  ...
  lapply(\(x) read_tsv(x, col_names = T, show_col_types = FALSE) %>% 
           mutate(
             sample = str_extract(x %>% basename(), 
                                  ".+?(?=_assemble|\\.assembly|\\.asm|_medaka|_polish|_links|_run|\\.spectra)"),
             stage = case_when(
...
for (i in 1:length(unique(merqury_stats$sample))) {
  ...
  cat(paste('## ', cur_sample),
      paste0('\n\n'),
      paste0('### { orientation="columns" .float .width=30%} \n'),
      ...
      glue::glue("{unique(highest_qv) %>% round(2)}%, at stage(s): {paste(highest_qv_stage, collapse = ', ')}"),
      paste0('\n'),
      paste0(':::'), 
      paste0('\n\n'),
      paste0('::: {.panel-tabset}'),
