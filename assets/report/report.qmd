...
quast_stats <- list.files(paste0(data_base, "quast"),
  ...
  mutate(
    sample = str_extract(
      assembly,
      ".+?(?=_assemble|\\.assembly|_medaka|_pilon|_links|\\.tigmint)"
    ),
    stage = case_when(
...
merqury_stats <- list.files(paste0(data_base, "merqury"), full.names = T, pattern = "stats") %>% 
  ...
  bind_rows() %>% 
  mutate(sample = str_extract(sample_stage,".+?(?=_assemble|\\.assembly|_medaka|_pilon|_links|\\.tigmint)"),
         stage = case_when(
...
merqury_asm_hists <- list.files(paste0(data_base, "/merqury"), full.names = T, pattern = "asm.hist")  %>% 
  lapply(\(x) read_tsv(x, col_names = T, show_col_types = FALSE) %>% 
           mutate(
             sample = str_extract(x %>% basename(), ".+?(?=_assemble|\\.assembly|_medaka|_polish|_links|_run|\\.spectra)"),
             stage = case_when(
